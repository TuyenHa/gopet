...
public class GameController
{
            ...
            case GopetCMD.ON_OTHER_USER_MOVE:
                {
                    ...
                    // Tối thiểu 2: bên dưới đọc points[len-2] và points[len-1].
                    // Tối đa 64: đường đi thật chỉ vài điểm — khối chống hack cũ
                    // (đã comment, ngay dưới) coi >30 là đáng ngờ. Mảng này còn
                    // được phát lại cho cả khu vực qua sendMove nên độ dài lớn
                    // thành đòn khuếch đại, không chỉ là tốn bộ nhớ.
                    int[] points = new int[message.reader().readArrayLength(2, 64, "ON_OTHER_USER_MOVE.points")];
    ...
    private void guider(sbyte subCMD, Message message)
    {
        switch (subCMD)
        {
            ...
            case GopetCMD.TYPE_DIALOG_INPUT:
                ...
                // Số ô nhập của một hộp thoại. InputReader còn kiểm lại phải khớp
                // đúng inputType.Length, nhưng phép kiểm đó chạy SAU khi đã cấp phát —
                // nên vẫn cần chặn ở đây. 32 là rộng rãi cho mọi hộp thoại hiện có.
                String[] texts = new String[message.reader().readArrayLength(0, 32, "TYPE_DIALOG_INPUT.texts")];
    ...
    private void showInfoPetUpTier(int petId1, int petId2)
    {
        ...
        if (petActive == null || petPassive == null || petActive == petPassive)
        {
            showDescPetUpTierUI(player.Language.CouldNotFoundPet, null);
            return;
        }
