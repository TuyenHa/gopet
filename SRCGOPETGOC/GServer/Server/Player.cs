...
{
    private readonly object _coinLock = new object();
    ...
    public virtual void onMessage(Message ms)
    {
        ...
        try
        {
            ...
            {
                // Gói tin sai cấu trúc (độ dài mảng ngoài khoảng, đọc quá cuối gói).
                // Client hợp lệ không bao giờ gửi thứ này — hoặc nó đang lỗi, hoặc
                // đang dò tìm lỗ hổng. Đóng kết nối thay vì để nó thử tiếp.
                string ip;
                try { ip = session.CSocket?.RemoteEndPoint?.ToString() ?? "?"; }
                catch { ip = "?"; }

                GopetManager.ServerMonitor.LogWarning(
                    $"Ngắt kết nối do gói tin sai cấu trúc từ {ip} (opcode {ms.id}): {e.Message}");
                session.Close();
            }
            catch (Exception e)
            {
...
Thread.Sleep(1000);
    ...
    public bool TrySpendCoin(long value)
    {
        if (value < 0) return false;
        lock (_coinLock)
        {
            if (playerData.coin < value) return false;
